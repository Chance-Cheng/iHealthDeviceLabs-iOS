...
/**
 * Start scan iHealth device with bluetooth low energy
 * @param tempDeviceType  iHealth DeviceType 
 * Before call the Api, should register discover Notification. such as :[[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(devicePO3Discover:) name:PO3Discover object:nil];
 * You will get two type result:
    ...
    Attentation: Stop scan befroe connect device, other connect may fail.
 */
